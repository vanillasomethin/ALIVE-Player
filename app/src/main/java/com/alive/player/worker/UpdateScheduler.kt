...
object UpdateScheduler {
    ...
    fun checkNow(context: Context) {
        ...
        WorkManager.getInstance(context).enqueueUniqueWork(
            ONESHOT_WORK_NAME,
            // KEEP: a second press while a check is running joins it — REPLACE would
            // cancel the running worker into a zombie whose download keeps streaming
            // to the same staging file the replacement then writes to.
